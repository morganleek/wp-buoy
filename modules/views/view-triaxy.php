<?php 
	// function uwa_triaxy_time_series_csv($buoy_id = '', $from_date = null, $until_date = null) {
	// 	if($buoy_id == '') {
	// 		return;
	// 	}
	...
	// function uwa_triaxy_time_series_data($buoy_id = '', $from_date = null, $until_date = null) {
	...
	// function uwa_triaxy_model_results($buoy_id, $from = null, $until = null) {
	...
	// 				$wpdb->prepare("SELECT * FROM `{$wpdb->prefix}triaxy_memplot` 
	// 					WHERE buoy_id='%s' 
	// 					" . $until_where . "
	// 					ORDER BY timestamp 
	// 					DESC LIMIT 1
	...
	// 						<img src="/wp-admin/admin-ajax.php?action=uwa_triaxy_aws&do=image_fetch&key=' . $memplot->url . '" alt="">
    ...
    // Get list of Buoys
		// $buoys = $wpdb->get_results("SELECT * FROM `{$wpdb->prefix}triaxy_post_data_processed_waves` GROUP BY `buoy_id`");
    $buoys = $wpdb->get_results(" SELECT `buoy_id` FROM `{$wpdb->prefix}buoy_info` WHERE `visible` = 1 AND `buoy_type` = 'triaxy'");
    _d($wpdb);
	...
	// 					<a style="float: right;" href="/triaxy?buoy_id=' . $b->buoy_id . '" class="btn btn-success" role="button">Go to ' . $title . ' Data Page</a>
	// 				</div>';
	// 				print '<div class="panel-body">';
	...
	// 				    // print '<a href="/triaxy?buoy_id=' . $b->buoy_id . '" class="btn btn-success" role="button">Go to ' . $title . ' Data Page</a>';
	// 					}
	// 				print '</div>';
	// 			print '</div>';
	// 		}		
